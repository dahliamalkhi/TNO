...
public final class SchemaField extends FieldProperties {
 ...
 /** Create a new SchemaField with the given name and type,
  ...
  public boolean encrypted() { return (properties & ENCRYPTED)!=0; }
  ...
  public void checkFieldCacheSource(QParser parser) throws SolrException {
    ...
    if ( encrypted() ) {
      throw new SolrException(SolrException.ErrorCode.BAD_REQUEST,
          "can not use FieldCache on encrypted field: "
              + getName());
    }
  }
  ...
  static int calcProps(String name, FieldType ft, Map<String,?> props) {
    ...
    if (on(trueProps,ENCRYPTED)) {
      int pp = (STORE_TERMVECTORS | STORE_TERMPOSITIONS | STORE_TERMOFFSETS | STORE_OFFSETS | MULTIVALUED | DOC_VALUES);
      if (on(pp,trueProps)) {
        throw new RuntimeException("SchemaField: " + name + " conflicting encrypted field options:" + props);
      }
      p &= ~pp;
      pp = (OMIT_NORMS | OMIT_TF_POSITIONS | OMIT_POSITIONS);
      if (on(pp,falseProps)) {
        throw new RuntimeException("SchemaField: " + name + " conflicting 'false' field options for encrypted field:" + props);
      }
      p |= pp;
    }
  ...
  public SimpleOrderedMap<Object> getNamedPropertyValues(boolean showDefaults) {
    ...
    if (showDefaults) {
      ...
      properties.add(getPropertyName(ENCRYPTED), encrypted());
